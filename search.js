...
function iterateRecordsForFilter(data) {
	...
	$.each(data.result.records, function(recordKey, recordValue) {
		...
		if(recordTitle) {
			if(154> parseFloat(recordLon)&&parseFloat(recordLon)>152.5 && -26.5>parseFloat(recordLat)&&parseFloat(recordLon)>-28){//distance:185614.2345889979 metres
				if( recordTitle.toUpperCase().indexOf(searchTerm.toUpperCase())>-1){
					$("#records").append(
						$('<section class="record">').append(
							$('<h2>').text(recordTitle),
							$('<h3>').text(recordLon),
							$('<h3>').text(recordLat),
							$('<h1>').text(dis), //distance from user's location to the library (km)
...
function iterateRecords(data) {
	...
	$.each(data.result.records, function(recordKey, recordValue) {
		...
		if(recordTitle) {
			if(154> parseFloat(recordLon)&&parseFloat(recordLon)>152.5 && -26.5>parseFloat(recordLat)&&parseFloat(recordLon)>-28){//distance:185614.2345889979 metres
				if( parseFloat(dis) < parseFloat(sliderValue)){
					$("#records").append(
						$('<section class="record">').append(
							$('<h2>').text(recordTitle),
							$('<h3>').text(recordLon),
